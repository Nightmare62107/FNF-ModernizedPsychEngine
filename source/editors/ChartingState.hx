...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		'GF Duet',
		'No Animation',
		'Player On Opponent Side',
		'Player And GF On Opponent Side',
		'Opponent On Player Side',
		'Opponent And GF On Player Side'
	];
	...
	[
		...
		['Camera Flash', "Value 1: Flash time (in seconds)\n\nLeaving Value 1 blank will have flash will last for\n0.5 seconds by default."],
		['Silhouette', "Value 1:\nOn = Fade In\nAnything Else = Fade Out\n\nValue 2: Silhouette Fade In/Out Time"],
		['Change Character', "Value 1: Character to change (Dad, BF, GF)\nValue 2: New character's name"],
		['Change Scroll Speed', "Value 1: Scroll Speed\nValue 2: Time it takes to change fully in seconds.\nValue 3: Type, \"constant\" or \"multiplicative\""],
	...
	**/
	...
	function addSongUI():Void
	{
		...
		var check_countdown = new FlxUICheckBox(saveButton.x - 30, saveButton.y + 60, null, null, "Skip Countdown", 100);
		check_countdown.checked = _song.skipCountdown;
		// _song.skipCountdown = check_countdown.checked;
		check_countdown.callback = function()
		{
			_song.skipCountdown = check_countdown.checked;
			//trace('CHECKED!');
		};
		...
		tab_group_song.add(check_countdown);
	...
	var readyCountdownInputText:FlxUIInputText;
	var setCountdownInputText:FlxUIInputText;
	var goCountdownInputText:FlxUIInputText;
	var healthBarInputText:FlxUIInputText;
	var timeBarInputText:FlxUIInputText;
	function addTexturesUI():Void
	{
		var skin = PlayState.SONG.arrowSkin;
		if(skin == null) skin = '';
		noteSkinInputText = new FlxUIInputText(10, 25, 120, skin, 8);
		blockPressWhileTypingOn.push(noteSkinInputText);

		noteSplashesInputText = new FlxUIInputText(noteSkinInputText.x + 140, noteSkinInputText.y, 120, _song.splashSkin, 8);
		blockPressWhileTypingOn.push(noteSplashesInputText);

		var healthBarSkin = PlayState.SONG.healthBarSkin;
		if(healthBarSkin == null) healthBarSkin = '';
		healthBarInputText = new FlxUIInputText(noteSkinInputText.x, noteSkinInputText.y + 35, 120, healthBarSkin, 8);
		blockPressWhileTypingOn.push(healthBarInputText);

		var timeBarSkin = PlayState.SONG.timeBarSkin;
		if(timeBarSkin == null) timeBarSkin = '';
		timeBarInputText = new FlxUIInputText(noteSplashesInputText.x, healthBarInputText.y, 120, timeBarSkin, 8);
		blockPressWhileTypingOn.push(timeBarInputText);

		var readySkin = PlayState.SONG.readySkin;
		if(readySkin == null) readySkin = '';
		readyCountdownInputText = new FlxUIInputText(noteSkinInputText.x, healthBarInputText.y + 35, 120, readySkin, 8);
		blockPressWhileTypingOn.push(readyCountdownInputText);

		var setSkin = PlayState.SONG.setSkin;
		if(setSkin == null) setSkin = '';
		setCountdownInputText = new FlxUIInputText(noteSkinInputText.x, readyCountdownInputText.y + 35, 120, setSkin, 8);
		blockPressWhileTypingOn.push(setCountdownInputText);

		var goSkin = PlayState.SONG.goSkin;
		if(goSkin == null) goSkin = '';
		goCountdownInputText = new FlxUIInputText(noteSkinInputText.x, setCountdownInputText.y + 35, 120, goSkin, 8);
		blockPressWhileTypingOn.push(goCountdownInputText);

		var reloadNotesButton:FlxButton = new FlxButton(110, 325, 'Change Textures', function()
		{
			_song.arrowSkin = noteSkinInputText.text;
			_song.readySkin = readyCountdownInputText.text;
			_song.setSkin = setCountdownInputText.text;
			_song.goSkin = goCountdownInputText.text;
			_song.healthBarSkin = healthBarInputText.text;
			_song.timeBarSkin = timeBarInputText.text;
			updateGrid();
		});
		reloadNotesButton.setGraphicSize(80, 30);
		reloadNotesButton.updateHitbox();

		var tab_group_textures = new FlxUI(null, UI_box);
		tab_group_textures.name = "Textures";
		tab_group_textures.add(reloadNotesButton);
		tab_group_textures.add(noteSkinInputText);
		tab_group_textures.add(noteSplashesInputText);
		tab_group_textures.add(readyCountdownInputText);
		tab_group_textures.add(setCountdownInputText);
		tab_group_textures.add(goCountdownInputText);
		tab_group_textures.add(healthBarInputText);
		tab_group_textures.add(timeBarInputText);
		tab_group_textures.add(new FlxText(noteSkinInputText.x, noteSkinInputText.y - 15, 0, 'Note Texture:'));
		tab_group_textures.add(new FlxText(noteSplashesInputText.x, noteSplashesInputText.y - 15, 0, 'Note Splashes Texture:'));
		tab_group_textures.add(new FlxText(readyCountdownInputText.x, readyCountdownInputText.y - 15, 0, 'Ready Countdown Texture:'));
		tab_group_textures.add(new FlxText(setCountdownInputText.x, setCountdownInputText.y - 15, 0, 'Set Countdown Texture:'));
		tab_group_textures.add(new FlxText(goCountdownInputText.x, goCountdownInputText.y - 15, 0, 'Go Countdown Texture:'));
		tab_group_textures.add(new FlxText(healthBarInputText.x, healthBarInputText.y - 15, 0, 'Health Bar Texture:'));
		tab_group_textures.add(new FlxText(timeBarInputText.x, timeBarInputText.y - 15, 0, 'Time Bar Texture:'));
		UI_box.addGroup(tab_group_textures);
	}
	...
	function addEventsUI():Void
	{
		...
		descText = new FlxText(20, 290, 0, eventStuff[0][0]);
		...
		else if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText))
		{
			...
			else if(curSelectedNote != null)
			{
				...
				else if(sender == value4InputText)
				{
					if(curSelectedNote[1][curEventSelected] != null)
					{
						curSelectedNote[1][curEventSelected][3] = value4InputText.text;
						updateGrid();
					}
				}
				else if(sender == value5InputText)
				{
					if(curSelectedNote[1][curEventSelected] != null)
					{
						curSelectedNote[1][curEventSelected][3] = value5InputText.text;
						updateGrid();
					}
				}
	...
	override function update(elapsed:Float)
	{
		...
		{
			if (touch.x > gridBG.x && touch.x < gridBG.x + gridBG.width && touch.y > gridBG.y && FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom])
			{
	...
	*/
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		else
		{
			...
			_song.events.push([noteStrum, [[event, text1, text2, text3, text4, text5]]]);
